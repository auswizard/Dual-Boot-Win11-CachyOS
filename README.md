...
4) Type these specific commands to create a 2GB 'System EFI Partition':
  diskpart
  list disk
  select disk x (where x is the number of the harddrive, eg: 'select disk0')
  convert gpt
  create partition efi size=2048
  format quick fs=fat32
  exit
5) Close the Command Prompt window and proceed with the installation.
6) When you get to the section on what HDD to select, you will have two partitions, select the one that says 'unallocated space'
  disk0 - 2.0gb System EFI Partition
  disk0 - unallocated space (xxxGB free space) <-------- Install to this partition.
7) Complete the Windows installation, when you get to the desktop feel free to complete initial setup steps, installing drivers and software if required.
